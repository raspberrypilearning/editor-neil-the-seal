...
Click on the `Stage`. Make two variables, `score`{:class="block3variables"} and `stuff to smash`{:class="block3variables"}, both **For all sprites**. Tick `score`{:class="block3variables"} so the player can see it, and untick `stuff to smash`{:class="block3variables"} to hide it.
...
Click on the `Sign`{:class="block3looks"} sprite. Inside the `repeat ()`{:class="block3control"} loop, add a `change stuff to smash by ()`{:class="block3variables"} block to count each clone. Then add a `wait () seconds`{:class="block3control"} block for `0.1` seconds at the very top of the script.
